...
Bot Guide（`botguide`），为"入行365"生态提供 Bot 使用指南与教程内容。

公开页面、教程文案、Bot 说明、任务说明和用户引导必须优先使用自然、具体、像真人说话的表达；避免 PRD 腔、技术黑话、过度抽象、堆概念、强营销口号和生硬的防御性声明。不能为了更口语而删掉核心路径、CTA、测试合同、合规边界或产品交付边界。
...
- 使用 `@tailwindcss/postcss` 集成 Tailwind CSS 4，样式优先使用 utility classes。
- 遵循 Next.js App Router 约定（`app/` 目录结构）。
